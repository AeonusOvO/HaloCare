...
const Layout: React.FC<Props> = ({ currentView, onChangeView, children }) => {
  const navItems = [
    ...
    { id: AppView.AI_DIAGNOSIS, label: '四诊辨证', icon: ScanFace, isCenter: true },
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f7f5f0] md:flex-row text-stone-800">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-emerald-900 text-emerald-50 shadow-2xl z-20">
        ...
        <nav className="flex-1 px-4 space-y-2">
          {navItems.map(item => (
            <button
              ...
              className={`motion-press w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                currentView === item.id
        ...
        <div className="p-6 text-xs leading-5 text-emerald-300/75 text-center">
          本系统仅用于健康管理和科研演示
        </div>
      </aside>
        ...
        {navItems.map(item => {
          ...
          if (item.isCenter) {
             return (
               <div key={item.id} className="relative -top-6 flex flex-col items-center justify-center">
                 <button
                    onClick={() => onChangeView(item.id)}
                    className={`motion-press w-16 h-16 rounded-full flex items-center justify-center shadow-lg border-4 border-[#f7f5f0] transition-all transform ${
                      isActive
          ...
          return (
            <button
              key={item.id}
              onClick={() => onChangeView(item.id)}
              className={`motion-press flex flex-col items-center justify-center p-2 flex-1 rounded-lg transition-colors ${
