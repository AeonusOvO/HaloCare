...
const Home: React.FC<Props> = ({ userProfile, onChangeView, token, userId }) => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 overflow-y-auto pb-24 h-full">
      <header className="motion-enter flex justify-between items-center mb-6 mt-2 relative z-20">
        <div>
           <div
             className="relative"
          ...
          <button
             ...
             className={`motion-press px-3 py-1 rounded-full text-xs font-bold border transition-colors ${isFamilyMode ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-stone-100 text-stone-500 border-stone-200'}`}
          >
      ...
      <div className="motion-enter motion-enter-delay-1 grid grid-cols-1 md:grid-cols-6 gap-4">
        <div className="motion-card md:col-span-4 bg-gradient-to-br from-emerald-800 to-teal-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
          <div className="motion-breathe absolute right-0 bottom-0 opacity-10 transform translate-x-10 translate-y-10">
            <ScanFace size={180} />
          </div>
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-emerald-500/30 text-emerald-100 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider">四诊入口</span>
            </div>
            <h2 className="text-xl font-bold mb-2">智能体质辨识</h2>
            <p className="text-emerald-100 text-sm mb-6 max-w-[80%]">
              通过望闻问切多模态分析，生成您的专属健康画像，获取个性化调理建议。
            </p>
            <div className="flex gap-3">
              <button
                ...
                className="motion-press bg-white text-emerald-900 px-5 py-2 rounded-full font-bold text-sm shadow-lg hover:bg-emerald-50 transition-colors"
              >
                开始辨证
              </button>
        ...
        <div className="motion-card md:col-span-2 bg-white rounded-2xl border border-stone-200 shadow-sm p-5 flex flex-col gap-3">
          ...
          <button
            ...
            className="motion-press mt-auto w-full py-2 bg-stone-800 text-white rounded-xl text-sm hover:bg-stone-700 transition"
          >
            查看详细趋势
          </button>
        </div>
      ...
      <div className="motion-enter motion-enter-delay-2 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div
            className="motion-card bg-white p-0 rounded-2xl border border-stone-200 shadow-sm overflow-hidden md:col-span-2 cursor-pointer"
            onClick={async () => {
          ...
          <div
            className="motion-card bg-white p-4 rounded-2xl border border-stone-200 shadow-sm cursor-pointer"
            onClick={async () => {
          ...
          <div
            className="motion-card bg-white p-4 rounded-2xl border border-stone-200 shadow-sm md:col-span-1 cursor-pointer"
            onClick={async () => {
      ...
      <div
        className="motion-enter motion-enter-delay-3 bg-white p-4 rounded-2xl border border-stone-200 shadow-sm"
        onClick={async () => {
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <button
            ...
            className="motion-press px-3 py-3 rounded-xl border border-stone-200 hover:border-emerald-300 hover:bg-emerald-50 text-stone-700 font-bold text-sm transition"
          >
            一键四诊辨证
          </button>
          <button
          ...
          <button
            ...
            className="motion-press px-3 py-3 rounded-xl border border-stone-200 hover:border-emerald-300 hover:bg-emerald-50 text-stone-700 font-bold text-sm transition flex items-center justify-center gap-2"
          >
